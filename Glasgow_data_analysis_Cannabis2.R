...
filename = paste("BarPlotAdoption_case_2.pdf")
pdf(filename )
...
barplot(sort(cannabis2[,4], decreasing=T), axes = T, space = 2, horiz = TRUE, xlim = c(0,60), names.arg = Labels, cex.names = 0.8, xlab= "frequencies",las=1, cex = 0.8, main = "")

dev.off()

# Transformation in binary entries 

#dataframe with same size as cannabis2
cannabis2.b1 = cannabis2
cannabis2.b1[cannabis2.b1==1] = 1
...
#  consistent entries with the model when 1 is  an adopter
index1 = NULL
...
filename = paste("BarPlotBinaryAdoptions_case_2.pdf")
pdf(filename )
...
barplot(sort(cannabis2.b2[,4], decreasing=T), axes = T, space = 0.5, horiz=TRUE,xlim = c(0,60),names.arg = Labels, cex.names = 1.1, cex = 1.1, xlab= "frequencies",las=1)
...
save(x_par12, x_par22, x_par32, M_par12, M_par22, M_par32, 
     file = "cannabis_data_ready_for_estimation2.RData")
